"""Live full-pipeline test: Taiwan -- TSMC (2330).

Wrappers tested:
  Micro: tw_mops_wrapper (MOPS) -- no key
  Macro: macro_dgbas (IMF SDMX fallback) -- no key
  OHLCV: twstock -- no key

Run: python3.12 -m pytest tests/test_live_tw_tsmc.py -v
"""
...
class TestTWFullPipeline(unittest.TestCase):
    """Full pipeline integration test for Taiwan -- TSMC (2330)."""
    ...
    def test_01_micro_mops_profile(self):
        try:
            ...
            self.skipTest(f"MOPS unavailable: {exc}")

    def test_02_ohlcv_twstock(self):
        df = fetch_ohlcv_safe(TICKER, MARKET_ID, years=1)
        if df is None or len(df) == 0:
            df = fetch_ohlcv_safe(YFINANCE_TICKER, "", years=1)
        self.assertIsInstance(df, pd.DataFrame)
        self.assertGreater(len(df), 0, "OHLCV should have data for TSMC")
        self.__class__._ohlcv = df

    def test_03_macro_dgbas(self):
