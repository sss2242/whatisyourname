"""Live full-pipeline test: Brazil -- Petrobras (PETR4).

Wrappers tested:
  Micro: br_cvm_wrapper (CVM) -- no key
  Macro: BCB (python-bcb) / wbgapi fallback -- no key
  OHLCV: yfinance (PETR4.SA) -- no key

Run: python3.12 -m pytest tests/test_live_br_petrobras.py -v
"""
...
TICKER = "PETR4"
YFINANCE_TICKER = "PETR4.SA"
MARKET_ID = "br_cvm"
COUNTRY = "BR"


@pytest.mark.skipif(not NETWORK_OK, reason="No network connectivity")
class TestBRFullPipeline(unittest.TestCase):
    """Full pipeline integration test for Brazil -- Petrobras (PETR4)."""
    ...
    def test_01_micro_cvm_profile(self):
        try:
            ...
            self.skipTest(f"CVM unavailable: {exc}")

    def test_02_ohlcv_yfinance(self):
        df = fetch_ohlcv_safe(YFINANCE_TICKER, MARKET_ID, years=1)
        self.assertIsInstance(df, pd.DataFrame)
        self.assertGreater(len(df), 0, "OHLCV should have data for Petrobras")
        self.__class__._ohlcv = df

    def test_03_macro_bcb(self):
