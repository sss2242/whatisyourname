...
class HKHkexClient:
    ...
    def extract_segment_data(self, identifier: str) -> dict:
        ...
        market-specific segment keywords for hk_hkex.
        """
        try:
            from operator1.clients.filing_discoverer import try_segment_extraction
            return try_segment_extraction(identifier, "hk_hkex")
        except Exception as exc:
            import logging
            logging.getLogger(__name__).debug(
                "HKHkexClient segment extraction failed for %s: %s", identifier, exc,
            )
