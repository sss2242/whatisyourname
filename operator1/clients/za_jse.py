...
def _parse_director_dealings_pdf(
    ...
    def extract_segment_data(self, identifier: str) -> dict:
        ...
        market-specific segment keywords for za_jse.
        """
        try:
            from operator1.clients.filing_discoverer import try_segment_extraction
            return try_segment_extraction(identifier, "za_jse")
        except Exception as exc:
            import logging
            logging.getLogger(__name__).debug(
                "ZAJseClient segment extraction failed for %s: %s", identifier, exc,
            )
