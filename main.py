...
""",
    ...
    # ------------------------------------------------------------------
    # Step 6: Temporal modeling via staged runner (all execution paths)
    # ------------------------------------------------------------------
    # The staged runner is the SINGLE execution engine for all temporal
    # models (stages 3-7). This eliminates the dual-path architecture
    # that caused 11+ parity bugs between inline and staged paths.
    # When --stage is set: runs only the requested sub-stages with checkpoints.
    # When --stage is NOT set: runs all stages 3-7 without disk checkpoints.
    if not args.skip_models:
        ...
        _save_checkpoints = bool(args.stage)  # only save to disk when --stage is set
        ...
        _ps.macro_data = macro_data if macro_data else {}
        _ps.macro_dataset = macro_dataset
        _ps.target_holders = target_holders
        _ps.target_insiders = target_insiders
        _ps.filing_calendar_result = filing_calendar_result
        _ps.event_calendar_result = locals().get("event_calendar_result")
        _ps.options_signal_result = locals().get("options_signal_result")
        ...
        logger.info("Step 6: Running temporal models via staged runner (spec=%s)...", _stage_spec)
        run_stages(_ps, _stage_spec, save_checkpoints=_save_checkpoints)

        # Copy ALL results back to local variables for Step 7+8.
        # This is the single authoritative copy-back list. When adding a
        # new model result to PipelineState, add it here too.
        cache = _ps.cache
        ...
        # All model result variables remain None (initialized above).
        # USS variables
        scenario_result = None
