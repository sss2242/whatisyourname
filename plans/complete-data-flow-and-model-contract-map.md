...
| S35 | frequency_separator.py | Mixed Q+A filings merged naively, producing flow variable mismatches (annual totals in quarterly windows -> 319% gross margin) | **FIXED (2026-05-02)** -- Upgraded with Bayesian frequency detection, Chow-Lin temporal disaggregation, Denton proportional reconciliation. Revenue-ratio-scaled backfill from lower to higher frequencies. |
| S36 | llm_base.py | No system prompts for LLM calls -- report generation produces hype-laden language | **FIXED (2026-05-02)** -- Added `config/llm_system_prompts.yml` with anti-hype guardrails for entity discovery, report generation, and data extraction. |
| S37 | main.py / cache | `shares_outstanding` and `market_cap` missing from daily cache -- PE, EV, fcf_yield, and cash adequacy survival floor all NaN | **FIXED (2026-05-07)** -- Inject `shares_outstanding` from profile dict into cache. Compute `market_cap = close * shares_outstanding` when not already present. |
| S38 | derived_variables.py / financial_health.py | PE, EV/EBITDA, fcf_yield, P/S ratios computed on daily-interpolated flow data (grossly distorted) | **FIXED (2026-05-07)** -- Use filing-frequency values for cross-type ratios (market price / flow variable). Forward-fill from Q/A pipeline results into daily cache. |
| S39 | stages/ | No frequency-first pipeline -- temporal models run on daily cache with distorted Q/A ratios | **FIXED (2026-05-07)** -- Added frequency-first pipeline architecture (Stage 2). Each frequency (A/Q/S/M/W/D) runs its own derived_vars + survival + FH + regime + forecast + MC. Fusion forward-fills correct Q/A ratios into daily cache BEFORE temporal models (Stage 3+). |
| S40 | derived_variables.py / survival_mode.py / financial_health.py | Hardcoded 252 trading days per year in volatility annualization, Altman Z, runway computation | **FIXED (2026-05-07)** -- Added frequency-aware formulas. `freq_constants.py` provides `steps_per_year(freq)` returning 252/52/12/4/2/1. Volatility scales by `sqrt(steps_per_year)`. Altman Z components x3 (EBIT/TA) and x5 (Revenue/TA) are flow variables -- now use annualized values at Q/A/S frequencies. Liquidity runway uses frequency-appropriate monthly burn rate. |
| S41 | forecasting.py / conformal.py | Point forecasts with no distributional shape -- conformal bands are symmetric even when return distribution is skewed | **FIXED (2026-05-07)** -- Added 6 Beyond Bands distributional forecasting methods: (1) Distributional Random Forest, (2) Quantile Regression Forest, (3) Conformalized Quantile Regression, (4) NGBoost, (5) GAMLSS, (6) Bayesian Neural Network. `fit_distributional()` integrated into forecasting cascade. MC barriers passed through `run_simulation()` for barrier-aware path generation. Scenario decomposition rendering in report + dashboard. |
| S42 | conformal.py / prediction_aggregator.py | Confidence bands inverted (lower > upper) for some variables; bands too wide/narrow without regime context | **FIXED (2026-05-06)** -- 6 expert uncertainty improvements: (1) ConformalPIDCalibrator with online alpha tracking, (2) IV blend weight calibration from realized coverage, (3) width caps at 3x base width, (4) asymmetric widening from return skewness, (5) regime-conditional width scaling, (6) event proximity widening. |
| S43 | signal_ic.py | Signal IC computation takes 270 seconds (blocks pipeline) | **FIXED (2026-05-06)** -- Vectorized Spearman IC computation using `scipy.stats.spearmanr` on full matrix instead of per-variable loop. 270s -> 1s (287x speedup). |
| S44 | hedge_fund/engine.py + advanced_methods.py | HF pipeline has 15 base metrics but no forensic quality or market-data-driven signals | **FIXED (2026-05-06)** -- 30-method upgrade in 9 phases: (P1) Piotroski F-Score, balance sheet velocity, earnings persistence, forensic cashflow, OU mean reversion, accruals rank, Altman Z''. (P2) GARCH vol term structure, insider alignment, capital cycle, earnings torpedo, VRP proxy. (P3) Fama-French alpha stub, implied cost of capital, Merton default, cross-asset regime. Multi-frequency HF metrics for Momentum, DCF, Leverage Stress. HF data sufficiency validation to prevent misleading grades. |
| S45 | stage7_integration.py | HF sub-stage 7.5 is monolithic (all 30+ methods in one sub-stage, 180s timeout) | **FIXED (2026-05-06)** -- Split into 4 sub-stages: 7.5.1 (base metrics + scorecard + position), 7.5.2 (advanced methods), 7.5.3 (multi-freq HF variants), 7.5.4 (fusion). Each with independent timeout. |
| S46 | forecasting.py / monte_carlo.py / ohlc_predictor.py / financial_health.py | 4 cascading backtest bugs: (1) MC drawdown NaN from burn-out contamination, (2) OHLC year-end $78.89 crash, (3) FH=28.97 for Apple, (4) survival non-monotonic 1d=0%/21d=99.8% | **FIXED (2026-05-09)** -- (1) `get_regime_distributions()` filters to return-scale values only (`|v|<1.0`), MC rejects burn-out distributions with `|mean|>1.0` or `std>1.0`. (2) Noise growth changed from linear `0.02*day` to `sqrt(day)/12`, survival_mult clamped to [1.0,1.5]. (3) Sector-aware FH baseline floors for technology/financial/comms sectors, gated by gross margin + positive FCF. (4) `SECTOR_SURVIVAL_OVERRIDES` dict + `get_sector_aware_thresholds()` propagate sector thresholds to MC (technology: current_ratio<0.7 instead of <1.0). |
| S47 | stage5_forward.py / main.py | Sector-aware survival thresholds from `survival_mode.py` not propagated to MC simulation or FH scoring | **FIXED (2026-05-09)** -- MC `run_monte_carlo()` now receives sector via `get_sector_aware_thresholds()`. `compute_financial_health()` now accepts `sector` parameter for baseline floor adjustments. |
| S48 | various | Deep scan fixes (2026-05-03 to 2026-05-06): forward pass KeyError 0, CopulaResult attribute mismatches, 4 broken models for pickle serialization, conformal residuals attribute name, 9 missing profile defaults, balance field gaps, dead code removal, NameError bugs, SEC EDGAR rate limiting | **FIXED** -- 30+ individual bug fixes across 15+ files from 4 deep scan passes. |
