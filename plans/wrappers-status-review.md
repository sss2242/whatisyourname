...
| 5 | **AU ASX** | BHP | **8 holders** (was 0) | **1 row** (8.05% ownership, HHI=0.285) | MarketScreener (primary) + ASX MarkitDigital (fallback) | **FIXED (2026-03-26)**: MarkitDigital announcements API returns max 5 items (hard API limit). Fix: added MarketScreener (marketscreener.com) as primary holder source. Uses DuckDuckGo slug resolution + curl_cffi to scrape institutional shareholder tables. Returns BlackRock (3.245%), State Street (1.12%), Commonwealth Super (1.084%), etc. Also fixed MarketScreener regex to reject CSS selectors and French labels. No yfinance dependency. | **FIXED** |
...
| 7 | **ZA JSE** | NPN/SBK/SOL | **3 holders** for SBK (was 0), **1 holder** for SOL (was 0), **0** for NPN (no holder SENS in 15 most recent) | **1 row** per company with director_count | JSE SENS director dealing PDFs (pdfplumber + regex) | **FIXED (2026-03-26)**: Three bugs: (1) Code checked `ann.get('Headline')` but API returns `FlashHeadline` -- keyword filter never matched. (2) Used `DateTimePublished` but API key is `AcknowledgeDateTime`. (3) `_parse_director_dealings_pdf()` only handled SOL format ("Transaction date:") but not SBK format ("Date of Transaction", "Prescribed Officer"). Fix: broadened regex patterns for both JSE PDF sub-formats. Added `_parse_sens_holder_pdfs()` for structured SENS PDF extraction. NPN returns 0 because all 15 SENS are repurchase updates with no holder content (API limitation: max 15 recent items). No yfinance dependency. | **FIXED** (SBK/SOL); NPN limited by SENS API |
